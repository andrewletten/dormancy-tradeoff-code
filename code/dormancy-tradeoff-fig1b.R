...
### Modify max growth rate parameter of gleaner to make growth dynamics -------
### identical to the active phase of the dormancy strategist when half sat 
### constant = 0.1 (bottom row of Fig 1b)  
parameters["mu1"] <- 0.07
...
ks_seq <- 0.1*2^(seq(0, 8, 1)) # Ks1 sequence (active phase)
...
fig1b <- ggplot(results %>% filter(md > 0.0001 & Ks1 < 25), 
